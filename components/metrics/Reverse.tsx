...
export default function ReverseModeGraph() {
  ...
  const dataPoints = useMemo(() => {
    ...
    return history
      .map((item, idx) => ({
        time: idx,
        value: item.status615?.Reverse ? 1 : 0,
      }))
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl hover:scale-[1.02] border border-gray-100">
        <div className="mb-6">
          ...
          <div className="flex items-center gap-4">
            <div className={`text-base font-semibold ${currentValue === 1 ? 'text-yellow-500' : 'text-gray-900'}`}>
          ...
          <svg width={width} height={height} className="border border-gray-200 rounded bg-white overflow-visible" style={{ display: 'block', margin: '0 auto' }}>
              ...
              .map((point, index) => (
                <g key={`${point.time}-${index}`}>
                  <circle
                    ...
                    cy={getY(point.value)}
                    r="4"
                    fill={point.value === 1 ? "#f59e0b" : "#e5e7eb"}
                    stroke="white"
                    strokeWidth="2"
            ...
            {dataPoints.length > 0 && (
              <circle
                ...
                r="6"
                fill={currentValue === 1 ? "#f59e0b" : "#e5e7eb"}
                stroke="white"
                strokeWidth="3"
                className="animate-pulse"
                style={{ filter: currentValue === 1 ? "drop-shadow(0 0 8px rgba(245, 158, 11, 0.6))" : "none" }}
              />
            )}
          </svg>
        </div>
