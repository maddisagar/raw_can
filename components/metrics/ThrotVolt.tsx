...
export default function ThrotVoltGraph() {
  ...
  // Throttle Voltage range (0V to 5V)
  const minValue = 0
  const maxValue = 5
  const timeWindow = 20

  useEffect(() => {
    const initialValue = 1 + Math.random() * 2 // Start around 1-3V
    setDataPoints([{ time: 0, value: initialValue }])
    intervalRef.current = setInterval(() => {
      setCurrentTime((prev) => {
        const newTime = prev + 1
        const lastValue = dataPoints.length > 0 ? dataPoints[dataPoints.length - 1].value : 2
        const variation = (Math.random() - 0.5) * 0.5 // ±0.25V variation
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 transition-all duration-300 hover:shadow-3xl hover:scale-[1.02] border border-gray-100">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-gray-900 mb-1">Throttle Voltage Monitor</h2>
          <div className="flex items-center gap-4">
            {/* <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-cyan-500 rounded-full animate-pulse"></div>
              <span className="text-sm text-gray-600">Live Data</span>
            </div> */}
            <div className="text-base font-semibold text-gray-900">{currentValue.toFixed(2)} V</div>
        ...
        <div className="relative">
          <svg width={width} height={height} className="border border-gray-200 rounded bg-white overflow-visible" style={{ display: 'block', margin: '0 auto' }}>
            ...
            <path
              ...
              style={{ filter: "drop-shadow(0 2px 4px rgba(6, 182, 212, 0.2))" }}
            />
            {dataPoints
              ...
              .map((point, index) => (
                <g key={`${point.time}-${index}`}>
                  ...
                  <circle
                    ...
                    r="12"
                    fill="transparent"
                    className="hover:fill-cyan-500 hover:fill-opacity-10 cursor-pointer transition-all duration-200"
                  >
                    <title>{`Time: ${point.time}s, Voltage: ${point.value.toFixed(2)} V`}</title>
                  </circle>
                </g>
              ))}
            {dataPoints.length > 0 && (
        ...
        {/* <div className="mt-4 flex justify-between items-center text-sm text-gray-500">
          <span>
            Voltage Range: {minValue} - {maxValue} V
          </span>
